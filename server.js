  ...
  if (req.method === "GET" && req.path === "/shift_daily") {
    ...
    try {
      ...
      Object.keys(req.query).forEach((key) => {
        if (key.startsWith("filter_")) {
          ...
          if (filterField === "day_type") {
            data = data.filter((item) => (filterValue === "true" && item.day_type === "Working Day") || (filterValue === "false" && item.day_type === "OFF"));
          } else if (filterField === "flexible_shift") {
            data = data.filter((item) => (filterValue === "true" && item.flexible_shift === "Yes") || (filterValue === "false" && item.flexible_shift === "No"));
          } else if (filterField === "status") {
            data = data.filter((item) => (filterValue === "true" && item.status === "Active") || (filterValue === "false" && item.status === "Inactive"));
          }
          
          else {
            // Convert to appropriate type for comparison
            const parsedFilterValue = filterValue === "true" ? true : filterValue === "false" ? false : filterValue;
      ...
      if (req.query.search) {
        ...
        data = data.filter((item) => item.shift_code.toLowerCase().includes(searchTerm));
      }
    ...
    if (orderColumn && orderValue !== '') {
      // Explicit ordering when order has a value
